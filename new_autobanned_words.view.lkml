...
(CASE WHEN regexp_contains(word , '^harassing$|^harassed$|^trolls$|^harass$|^troll$|^gomi$|^tolerated$|^gofundme$|^tolerate$|^a\b$/b$$|^fuhrer$|^threatened$|^fucking$|^suicide$|^fucker$|^ssn$|^fascist$|^sexist$|^fascism$|^sex$|^facist$|^retard$|^facism$|^racist$|^dwlz$|^racial$|^diuretics$|^prostitute$|^dietbet$|^promo$|^dictator$|^petition$|^dick$|^oppressor$|^cunt$|^objectionable$|^coupon$|^Nazism$|^consensual$|^motherfucking$|^code$|^login$|^cock$|^impersonation$|^censorship$|^hater$|^censor$|^harassment$|^bullying$|^trolling$|^bully$|^threatening$|^bulimia$|^stalk$|^bulemia$|^sexism$|^bitchface$|^rape$|^bitch$|^r\b+f$|^bilemia$|^privacy$|^belimia$|^oppressive$|^bbb$|^nazi$|^asshole$|^lawsuit$|^ass$|^hateful$|^anorexic$|^tolerating$|^anorexia$|^slut$|^annorexic$|^racism$|^annorexia$|^opressed$|^annerexic$|^motherfucker$|^annerexia$|^tyrant$|^anerexic$|^retarded$|^anerexia$|^nonconsensual$|^abusive$|^threaten$|^abuser$|^hooker$|^abused$|^promotion$|^abuse$')
    then also_subquery.word
      ...
      END) as word ,
also_subquery.locale as locale,
COUNT(DISTINCT(CASE WHEN ( also_subquery.row_rank =1 and also_subquery.flagged_count=0 AND also_subquery.is_invisible is true and also_subquery.flagged IS TRUE) THEN also_subquery.post_id END)) AS auto_nomanageraction,
COUNT(DISTINCT(CASE WHEN (also_subquery.row_rank =1 and  also_subquery.flagged_count=0 AND also_subquery.is_invisible is true and also_subquery.flagged IS FALSE) THEN also_subquery.post_id END)) AS auto_banned ,
...
COUNT(DISTINCT (CASE WHEN (also_subquery.row_rank = 1 and also_subquery.flagged_count =0 and also_subquery.is_invisible is FALSE AND also_subquery.flagged is TRUE) then also_subquery.post_id END)) AS actionneeded_noaction,
COUNT(DISTINCT (CASE WHEN (also_subquery.row_rank=1 AND also_subquery.flagged_count=0 and also_subquery.is_invisible IS FALSE AND also_subquery.flagged is FALSE) THEN also_subquery.post_id END)) as not_reported_orflagged,
...
SELECT DISTINCT
  ...
  FROM
    ...
    payload_post.content as full_content,
    REGEXP_EXTRACT_ALL(LOWER(payload_post.content), r'[a-z][a-z][a-z]+\'?[a-z]?') one_words,
    REGEXP_EXTRACT_ALL(LOWER(payload_post.content), r'[a-z][a-z][a-z]+\'?[a-z]?[ ][a-z][a-z][a-z]+\'?[a-z]?') two_words,
    ...
    AND payload_post.created_at  > TIMESTAMP('2018-11-19 00:00:01')
    and payload_post.locale = 'en-US'
...
GROUP BY 1,2,3 ;;
  ...
  measure: not_reported_orflagged {
    type: sum
