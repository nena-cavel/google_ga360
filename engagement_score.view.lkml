...
SUM(CASE WHEN ((test.screen_name = 'connect_profile' OR test.screen_name = 'profile_other_view') AND test.hit_type = 'APPVIEW') THEN test.total_screenviews ELSE 0 END) AS view_profile,
...
FROM
 ...
 SELECT DISTINCT
  ...
  and (REGEXP_CONTAINS(h.appinfo.screenName, 'profile_other_view|connect_replies_list|connect_stream_trending|connect_profile|connect_comments|connect_stream_hashtag')
